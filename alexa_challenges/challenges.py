...
def launch():
    ...
    info_logger('Welcome to the trt quiz. Game is started')
...
def select_category(category):
    try:
        ...
        if category.lower() not in CATEGORY.keys():
            ...
            session.attributes['quiz_data'] = middleware(players*5, CATEGORY[category])
            quiz_instance = Quiz(session.attributes)
...
def quiz_beginning(ready_or_not):
    try:
        ...
        players = int(session.attributes['total_players'])
        quiz_data = middleware(players*5, 9)
